...
void board_init_history_step_records(struct board* board)
{
	board->history_step_records_capacity = INIT_HISTORY_STEPS_RECORD_SIZE;
	board->history_step_records = 
		(struct step**)malloc(sizeof(struct step*) * board->history_step_records_capacity);
	memset(board->history_step_records, 0, sizeof(struct steps*) * board->history_step_records_capacity);
	for (int i = 0; i < board->history_step_records_capacity; ++i)
	{
		board->history_step_records[i] = step_create();
	}
	board->history_step_records_size = 0;
}
...
struct board* board_init()
{
	...
	board_init_history_step_records(board);
...
void history_step_records_release(struct board* board)
{
	for (int i = 0; i < board->history_step_records_capacity; ++i)
	{
		step_release(board->history_step_records[i]);
	}
	free(board->history_step_records);
}
...
void board_release(struct board* board)
{
	...
	history_step_records_release(board);
...
int generate_moves(struct board* board, const struct piece* piece, int* mvs, int capatured)
{
	...
	if (piece->type == PIECE_TYPE_KING)
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			if (compare_piece_opponent_side(piece, board->pieces[dest]))
			{
				mvs[nums++] = get_move(piece->pos, dest);
			}
		}
	...
	else if (piece->type == PIECE_TYPE_CANNON)
	{
		for (int i = 0; i < 4; ++i)
		{
			...
			while (in_board(dest))
			{
				if (!board->pieces[dest]) dest += nDelta;
				else
				{
					if (compare_piece_opponent_side(piece, board->pieces[dest]))
					{
	...
	else if (piece->type == PIECE_TYPE_PAWN)
	{
		...
		if (in_board(dest))
		{
			...
			if (!capatured && !board->pieces[dest])
			{
				mvs[nums++] = get_move(piece->pos, dest);
			}
		}
...
int generate_all_moves_noncheck(struct board* board, int* mvs, int capatured)
{
	...
	for (int i = 0; i < 16; ++i)
	{
		n += generate_moves(board, pieces[i], &temp[n], capatured);
	}
	for (int i = 0; i < n; ++i)
	{
...
int generate_all_moves(struct board* board, int* mvs, int capatured)
{
	...
	for (int i = 0; i < 16; ++i)
	{
		nums += generate_moves(board, pieces[i], &mvs[nums], capatured);
	}
	return nums;
}
...
int no_way_to_move(struct board* board)
{
	...
	int n = generate_all_moves_noncheck(board, mvs, 0);
	return n == 0;
}
...
int legal_move_piece(struct board* board, struct piece* piece, int dest)
{
	...
	else if (piece->type == PIECE_TYPE_BISHOP)
	{
		return same_half(piece->pos, dest) &&
					 ...
					 !board->pieces[((piece->pos + dest) >> 1)];
	}
	else if (piece->type == PIECE_TYPE_KNIGHT)
	{
		return piece->pos != piece->pos + array_knight_pin[dest - piece->pos + 256] && 
					 !board->pieces[(piece->pos + array_knight_pin[dest - piece->pos + 256])];
	}
	else if (piece->type == PIECE_TYPE_KING || piece->type == PIECE_TYPE_ROOK || piece->type == PIECE_TYPE_CANNON)
	{
		...
		else
		{
			...
			while (curPos != dest && !board->pieces[curPos])
			{
				curPos += offset;
			}
			if (curPos == dest)
			{
				if (piece->type == PIECE_TYPE_KING)
				{
					return board->pieces[dest] && board->pieces[dest]->type == PIECE_TYPE_KING;
				}
				else if (piece->type == PIECE_TYPE_ROOK)
				{
					return !compare_piece_same_side(piece, board->pieces[dest]);
				}
				else
				{
					return !board->pieces[dest];
				}
			}
			else
			{
...
{
	if (board->history_step_records_size == 0) 
		return;

	struct step* step = board->history_step_records[--board->history_step_records_size];
	int mv = step->mv;
	int start = start_of_move(mv);
	int end = end_of_move(mv);
...
{
	// 历史表容量已满则扩增
	if (board->history_step_records_size == board->history_step_records_capacity)
	{
		int new_capacity = (board->history_step_records_capacity << 1);
		struct step** steps_record =
			(struct step**)malloc(sizeof(struct Step*) * new_capacity);
		memset(steps_record, 0, sizeof(struct steps*) * new_capacity);
		board->history_step_records_capacity = new_capacity;
		int i = 0;
		for (; i < board->history_step_records_size; ++i)
		{
			steps_record[i] = board->history_step_records[i];
		}
		for (; i < board->history_step_records_capacity; ++i)
		{
			steps_record[i] = step_create();
		}
		free(board->history_step_records);
		board->history_step_records = steps_record;
	}
...
int repetition_status(struct board* board, int recur)
{
	...
	for(int i = board->history_step_records_size - 1; i >= 0; --i)
	{
		struct step* step = board->history_step_records[i];
