...
int main()
{
	...
	while ((n = board->generateAllMovesNoncheck<GENERAL>(mvs)) > 0)
	{
		//int capatured_n = generate_all_moves(board, capatured_mvs, 1);
		int capatured_n = board->generateAllMovesNoncheck<GENERAL>(capatured_mvs);

		mboard = board->getMirrorBoard();
		exchange_board = board->getExchangeSideBoard();
		
		int mirror_n = mboard->generateAllMovesNoncheck<GENERAL>(mirror_mvs);
		int mirror_capatured_n= mboard->generateAllMovesNoncheck<GENERAL>(mirror_capatured_mvs);

		int exchange_n = exchange_board->generateAllMovesNoncheck<GENERAL>(exchange_mvs);
		int exchange_capatured_n = exchange_board->generateAllMovesNoncheck<GENERAL>(exchange_capatured_mvs);
